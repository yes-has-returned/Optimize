...
playerspritedict = {
    ...
    "player left attack hitbox sprite":pygame.transform.flip(ProcessPSprite("Player L_R attack hitbox"), True, False),
    "player up attack hitbox sprite":ProcessPSprite("Player U_D attack hitbox"),
    "player down attack hitbox sprite":pygame.transform.flip(ProcessPSprite("Player U_D attack hitbox"), False, True),
}
...
class Player:
    ...
    def move(self, keypress, jkeypress, dashpress, attackpress, opress, uprecoil, friction=10):
        ...
        if attackpress == True and self.attack_cd == 0:
            if self.facing == "left":
                ...
                attackrect = Rectangle(Point(self.x2d-48, self.y2d), Point(self.x2d, self.y2d+32))
            elif self.facing == "right":
                self.attack_pos = (self.x2d+32, self.y2d)
                attackrect = Rectangle(Point(self.x2d+32, self.y2d), Point(self.x2d+80, self.y2d+32))
            elif self.facing == "up":
                self.attack_pos = (self.x2d - 16, self.y2d+24)
                attackrect = Rectangle(Point(self.x2d-16, self.y2d+24), Point(self.x2d+48, self.y2d+48))
            elif self.facing == "down":
                self.attack_pos = (self.x2d - 16, self.y2d - 32)
                attackrect = Rectangle(Point(self.x2d-16, self.y2d-32), Point(self.x2d+48, self.y2d))
...
class Enemy:
    ...
    def move(self, hitdir, attackrect, friction=10):
        self.hitbox = Rectangle(Point(self.xcor, self.ycor), Point(self.xcor+32, self.ycor+32))
        if attackrect != None:
...
while running:
    if testing_mode == "navigation":
        ...
        screen.blit(dummy1.spritedict[dummy1.current_sprite], (dummy1.xcor, 688-dummy1.ycor))
        keys = pygame.key.get_pressed()
        ...
        attackrect = P.move(keypress, jkeypress, dashpress, attackpress, opress, uprecoil)
        uprecoil = dummy1.move(P.facing, attackrect)

        if P.attack_pos != None:
            screen.blit(P.sprites[P.attack_sprite], (P.attack_pos[0], 688-P.attack_pos[1]))
